...
class CRCVV54Block:
    def __init__(self,cfg:V54RuntimeConfig|None=None):
        ...
        if self.cfg.suppression_enabled or self.cfg.width_enabled or self.cfg.joint_training_enabled:raise RuntimeError('V5.4 release requires suppression/width/joint-training disabled')
        if self.cfg.recovery_enabled and (not self.cfg.proposal_qualified or not self.cfg.verifier_qualified):raise RuntimeError('unqualified V5.4 recovery cannot affect runtime output')
    def refine(self,base_mask:np.ndarray):
        base=np.asarray(base_mask).astype(np.uint8)
        if not self.cfg.recovery_enabled:return base.copy(),{'policy':'fail_closed','proposal_qualified':self.cfg.proposal_qualified,'verifier_qualified':self.cfg.verifier_qualified,'recovery_applied':False}
