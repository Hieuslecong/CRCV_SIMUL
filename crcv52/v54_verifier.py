...
def prepare_context(record):
    image=record['image'].astype(np.float32);prob=record['prob'].astype(np.float32);base=record['base'].astype(bool);thr=float(record['threshold']);bh,dark,gr=evidence(image);gray=cv2.cvtColor((np.clip(image,0,1)*255).astype(np.uint8),cv2.COLOR_RGB2GRAY).astype(np.float32)/255.;rp=relprob(prob,thr);n,lab=cv2.connectedComponents(base.astype(np.uint8),8);return {'image':image,'prob':prob,'base':base,'thr':thr,'maps':(rp,bh,dark,gr,gray),'labels':lab,'foreign_dist':{}}
def _stats(a,m):
    v=a[m]
...
def descriptor(record,fam,z,ctx=None):
    ...
    if fam=='v52b':sy,sx=map(int,z['candidate']['source_yx']);hist=np.asarray(z['candidate']['history'],np.float32)
    else:sy,sx=map(int,z.get('source_yx',(0,0)));hist=np.asarray(z.get('hist_xy',[]),np.float32)
    src=np.zeros_like(base,np.uint8)
    if 0<=sy<base.shape[0] and 0<=sx<base.shape[1]:cv2.circle(src,(sx,sy),4,1,-1)
    src=src.astype(bool)&base;ring=cv2.dilate(add.astype(np.uint8),np.ones((5,5),np.uint8),1).astype(bool)&~add;raw_score=float(z.get('score',z.get('pair_score',0.)));mean_field=float(z.get('mean_field',z.get('mean_pair_field',0.)));min_field=float(z.get('min_field',z.get('min_pair_field',0.)));mean_ridge=float(z.get('mean_ridge',0.));min_ridge=float(z.get('min_ridge',0.));length=float(z.get('length',int(add.sum())));source_score=float(z.get('source_score',0.));dest_score=float(z.get('destination_score',0.));pair_score=float(z.get('pair_score',0.));pair_dist=float(z.get('pair_distance',0.));fa=float(z.get('facing_a',0.));fb=float(z.get('facing_b',0.));identity=float(z.get('min_identity',0.));stop=float(z.get('stop_prob',0.));field_margin=float(z.get('field_margin',0.));fam_one=[1. if fam==f else 0. for f in FAMS];pxy=np.asarray([(x,y) for y,x in z.get('path_yx',[])],np.float32)
    ...
    fd=c['foreign_dist'][cid];end_yx=(int(ys[-1]),int(xs[-1])) if len(ys) else (sy,sx);end_dist=float(fd[end_yx]);touch=cv2.dilate(add.astype(np.uint8),np.ones((3,3),np.uint8),1).astype(bool)&base;ids={int(v) for v in np.unique(lab[touch]) if int(v)>0 and int(v)!=cid};f=fam_one+[raw_score,mean_field,min_field,field_margin,mean_ridge,min_ridge,identity,stop,source_score,dest_score,pair_score,min(pair_dist/64.,2.),fa,fb,min(length/64.,2.),straight,mean_turn,max_turn,min(end_dist/32.,3.),min(len(ids)/3.,2.),float(bool(ids)),float(fam=='bidir_connect')]
    for a in c['maps']:
