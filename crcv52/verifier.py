...
def descriptor(image,prob,base,thr,z):
    ...
    src=np.zeros_like(base,np.uint8);cv2.circle(src,(sx,sy),4,1,-1);src=src.astype(bool)&base;corr=q['corridor']&~base;f=[z['score'],z['mean_field'],z['min_field'],z['target_score'],float(z.get('field_margin',0.)),min(add.sum()/32.,3.),q['horizon']/24.,abs(q['angle'])/8.,np.log1p(comp.sum())/8.]
    ...
    f += [compat,straight];return np.asarray(f,np.float32)
