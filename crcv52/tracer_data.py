...
def _nearest_missing_seed(missing,ep,history_xy=None,radius=8):
    y,x=ep;best=None;outv=None
    if history_xy is not None and len(history_xy)>=2:
        v=np.asarray(history_xy[-1]-history_xy[-2],float);outv=v/(np.linalg.norm(v)+1e-9)
...
def trace_missing_from_endpoint(gt,base,ep,max_len=40,seed_radius=8,border_margin=8):
    ...
    anchor,hist=boundary_exit_anchor(base,ep,hist,12);gsk=skeletonize(gt.astype(bool));missing=gsk&~base.astype(bool);seed=_nearest_missing_seed(missing,anchor,hist,seed_radius)
    if seed is None:return []
    path=[seed];prev=anchor;cur=seed;prev_vec=np.array([seed[0]-anchor[0],seed[1]-anchor[1]],float)
    for _ in range(max_len-1):
        ns=[q for q in _neighbors(gsk,cur) if q!=prev];missing_ns=[q for q in ns if missing[q]]
        if not missing_ns:break
        if len(missing_ns)>1:
            pv=prev_vec;pn=np.linalg.norm(pv)+1e-9
            def sc(q):
                v=np.array([q[0]-cur[0],q[1]-cur[1]],float);return float(np.dot(pv,v)/(pn*(np.linalg.norm(v)+1e-9)))
    ...
    for r in records:
        ...
        for ep in endpoints(r['base']):
            ...
            if len(path)>=min_len:
                ...
                if hist is not None:
                    anchor,hist2=boundary_exit_anchor(r['base'],ep,hist,12);events.append({'name':r['name'],'lineage':r['lineage'],'source_endpoint_yx':ep,'source_yx':anchor,'history_xy':hist2,'path_yx':path,'record':r})
    return events
