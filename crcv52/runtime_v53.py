...
class V53RuntimeConfig:
    ...
    def load(cls, path: str | Path = Path(__file__).resolve().parents[1] / "config_v53.json"):
        ...
        return cls(
            ...
            verifier_qualified=bool(d["verifier"]["qualified"]),
...
class CRCVV53Block:
    """Fail-closed V5.3 runtime facade.

    Current V5.3 proposal research is not accepted into the deployed output because
    the CAL verifier safety gate fails. Proposal generation remains diagnostic-only.
    """
    def __init__(self,cfg:V53RuntimeConfig|None=None):
        self.cfg=cfg or V53RuntimeConfig.load()
        if self.cfg.width_enabled or self.cfg.suppression_enabled or self.cfg.joint_training_enabled:
            raise RuntimeError("V5.3 release requires suppression/width/joint-training to remain disabled")
        if self.cfg.recovery_enabled and not self.cfg.verifier_qualified:
            raise RuntimeError("unqualified V5.3 recovery cannot influence runtime output")

    def refine(self, base_mask: np.ndarray):
        base=np.asarray(base_mask).astype(np.uint8)
        if not self.cfg.recovery_enabled:
            return base.copy(), {"policy":"fail_closed", "recovery_applied":False}
