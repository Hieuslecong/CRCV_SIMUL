...
def prepare_context(record):
    ...
    rp=relprob(prob,thr);bh,dark,gr=evidence(image);gray=cv2.cvtColor((np.clip(image,0,1)*255).astype(np.uint8),cv2.COLOR_RGB2GRAY).astype(np.float32)/255.;_,lab=cv2.connectedComponents(base.astype(np.uint8),8)
    return {'base':base,'maps':[rp,bh,dark,gr,gray],'labels':lab,'fd_cache':{},'hist_cache':{},'src_cache':{}}
...
def descriptor(record,z,ctx=None):
    ...
    src=c['src_cache'][src_key];ring=cv2.dilate(path.astype(np.uint8),np.ones((5,5),np.uint8),1).astype(bool)&~path
    lab=c['labels'];cid=int(lab[sy,sx]) if 0<=sy<H and 0<=sx<W else 0;comp=lab==cid if cid>0 else np.zeros_like(base)
    ...
    fd=c['fd_cache'][cid];end=(int(ys[-1]),int(xs[-1])) if len(ys) else (sy,sx);start_dist=float(fd[sy,sx]) if fd is not None else 99.;end_dist=float(fd[end]) if fd is not None else 99.;approach=np.clip((start_dist-end_dist)/20.,-3,3)
    pxy=np.asarray([(x,y) for y,x in z.get('path_yx',[])],np.float32);straight=mean_turn=max_turn=start_compat=end_compat=0.
    if len(pxy)>=2:
    ...
    f=[float(z['score']),float(z['min_identity']),float(z['stop_prob']),float(z['mean_field']),float(z['min_field']),float(z['source_score']),min(float(z['length'])/32.,2.),float(z.get('connects_foreign',False)),float(z.get('learned_stop',False)),approach,min(end_dist/30.,3.),min(float(comp.sum())/5000.,3.),straight,mean_turn,max_turn,start_compat,end_compat,
       float(z.get('consensus_mean',0.)),float(z.get('consensus_min',0.)),min(float(z.get('source_candidate_count',0))/64.,3.),float(z.get('rel_score',0.)),float(z.get('rel_min_identity',0.)),float(z.get('rel_stop_prob',0.)),float(z.get('rel_mean_field',0.)),float(z.get('rel_min_field',0.)),float(z.get('rel_length',0.))]
    for a in c['maps']:
