...
class Q1Protocol:
    ...
    def validate(self) -> "Q1Protocol":
        ...
        if self.min_reference_backbones < 2: raise ValueError("at least two canonical/reference backbones are required")
        if self.min_datasets < 3 or self.min_external_datasets < 1: raise ValueError("multi-dataset and external evidence cannot be weakened")
        if self.min_cross_dataset_routes < 2: raise ValueError("at least two cross-dataset routes are required")
        if not 0 < self.pair_positive_rate_floor <= 1: raise ValueError("bad positive-pair floor")
        if not 0 < self.corrected_p_floor < 1: raise ValueError("bad corrected-p threshold")
        return self


def _jsonable_protocol(p: Q1Protocol) -> dict:
    d=asdict(p); return {k:list(v) if isinstance(v,tuple) else v for k,v in d.items()}


def hash_protocol(p: Q1Protocol) -> str:
    return hashlib.sha256(json.dumps(_jsonable_protocol(p),sort_keys=True,separators=(",",":")).encode()).hexdigest()


def save_protocol(path: str | Path) -> dict:
    p=Q1Protocol().validate(); d=_jsonable_protocol(p); d["protocol_sha256"]=hash_protocol(p); Path(path).write_text(json.dumps(d,indent=2)+"\n"); return d
