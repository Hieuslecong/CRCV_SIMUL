# CRCV V5.21-dev3 — Compact Dual-Action Corrective Refinement

CRCV V5.21-dev3 is the compact scientific development candidate for corrective crack-segmentation refinement over **frozen Base probability maps**.
...
V5.21-dev3 promotes the supplied/tested `crcv_core` implementation (core implementation version `1.1.2`) as the canonical development path because it gives a complete bidirectional ADD/REMOVE correction hypothesis while remaining compact and GT-free at runtime.
...
The embedded core implementation version is `1.1.2`; the repository experiment-family version is `5.21.0.dev3`.

## Canonical real-data training

From the repository root, install the canonical package once with `python -m pip install -e .`, then run `python scripts/train_v521_real.py --manifest MANIFEST.csv --base-artifact BASE.ckpt --base-threshold 0.5 --git-commit GIT_SHA --dataset DATASET --backbone BACKBONE --resolution 256 --out RUN_DIR`. This is the only canonical V5.21 training entry point. The manifest contains **FIT/CAL/VAL only** with columns `name,split,source,lineage,image,mask,probability,base_artifact_sha256`; TEST/final rows are rejected. Images are normalized to `[0,1]`, probability maps must already be registered to the masks and lie in `[0,1]`, and lineages/exact image bytes may not cross FIT/CAL/VAL. Every row must bind its probability cache to the SHA256 of the frozen Base artifact.

The runner trains only on FIT natural frozen-Base errors, selects AAC thresholds on CAL under the existing TCRR safety cap, qualifies the combined ADD/REMOVE correction on VAL, and saves `add_model.txt`, `remove_model.txt`, and a Q1-compatible `run.json` with Git commit, dataset/backbone/resolution identity, manifest/content hashes, Base-artifact hash, and output artifact hashes. `NO_OP_CAL` or `NO_OP_VAL` is a valid fail-closed outcome and must not be overridden using TEST.
...
Deployment is fail-closed: `crcv_core.runtime.refine(...)` returns exact Base unless the literal boolean `qualified=True` is passed after reading an `ACTIVE` VAL qualification from `run.json`; truthy strings/status values do not activate mutation.
